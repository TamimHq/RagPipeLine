...
Here are some sample queries you can use to test the system, along with the expected (or actual working) outputs based on the `HSC26-Bangla1st-Paper.pdf` knowledge base and the final pipeline configuration.

**Note:** The system is designed to provide answers grounded in the retrieved context. If information is not present or not retrieved effectively, the system will state that it doesn't know.
